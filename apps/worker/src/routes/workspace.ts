...
workspaceRoutes.put(
  ...
  async (c) => {
    ...
    if (body.requireApprovalToPublish !== undefined) policy.requireApprovalToPublish = body.requireApprovalToPublish;
...
workspaceRoutes.put(
  ...
  async (c) => {
    ...
    await c.env.DB.prepare(
      'UPDATE workspaces SET ai_config_json = ? WHERE id = ?',
    )
